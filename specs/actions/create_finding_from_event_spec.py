...
with description(securecscc.CreateFindingFromEvent) as self:
    ...
    with it('sends finding to Google Cloud Security Command Center'):
        finding = self.action.run(fixtures.event())

        expect(self.gcloud_client.create_finding)\
            .to(have_been_called_with(self.organization, finding))
    ...
    with context('when receving a host event'):
        ...
        with it('includes instance id in asset ids'):
            ...
            finding = self.action.run(fixtures.event_host())
