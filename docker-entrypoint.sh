...
if [ "$1" = 'sysdig_secure_webhook' ]; then
    exec gunicorn -b 0.0.0.0:8080 --access-logfile - connectors.sysdig_secure_webhook:app
fi

if [ "$1" = 'falco_webhook' ]; then
    exec gunicorn -b 0.0.0.0:8080 --access-logfile - connectors.falco_webhook:app
fi
